...
void ball_move()
{
    if (isShowingGoalAnimation)
        return;
    if (kick_off == 0)
    {
        ball_x = 500;
        if (ball_y >= 200 || ball_y <= head_radius)
            dy = -dy;
        ball_y += dy;
        goal = 0;
        goalanicount = 0;
    ...
    if (ball_touched_ceil == 0 && ball_touched_ground == 0 && ball_shoot == 0 && kick_off == 1)
        ball_y = (ball_x - ball_x1 - 300) * (ball_x - ball_x1 - 300) / (-450) + ball_y1 + 200;
    else if (ball_touched_ground == 1)
        ball_y = (ball_x - ball_x1 - 300) * (ball_x - ball_x1 - 300) / (-900) + ball_y1 + 100;
...
void iDraw()
{
    ...
    else if (currentState == LEADREBOARD)
    {
        ...
        if (flag == 0)
        {
            update_leaderboard(-1, player1.name, player2.name);
            flag = 1;
        }
    ...
    else if (currentState == GAME)
    {
        ...
        if (time_count > 90 /*||(isGoldenGoal==1&&(scoreP1>=2||scoreP2>=2))*/)
        {
            ...
            if (leaderboard_update == 0)
                update_leaderboard(goal_diff, player1.name, player2.name);
...
{
    if (currentState == LEADREBOARD && key == 'b')
    {
        currentState = MENU;
        iResumeTimer(0);
    }
    ...
    if (currentState == GAME)
    {
        ...
        case GLUT_KEY_RIGHT:
        {
            if (head_x2 <= SCREEN_WIDTH - 30)
                ...
                head_x2 = SCREEN_WIDTH - head_radius;
            break;
        }
        default:
            break;
