...
else if(currentState == GOLDEN_GOAL) {
        iSetColor(255, 255, 0);
        iText(400, 500, "Golden Goal Match", GLUT_BITMAP_TIMES_ROMAN_24);
        iSetColor(255, 255, 255);
        iText(200, 450, "First to score wins");
        iText(200, 400, "Press 'b' to go back to Menu");
    }
...
if(!isGoldenGoal)    {if(time_count<=90.00){iText(660,580,time,GLUT_BITMAP_HELVETICA_18);}
...
if(isGoldenGoal){
    if(scoreP1==5||scoreP2==5) current_game=0;
     char win_text [50];
    if(scoreP1>scoreP2)
    {
    strcpy(win_text,player1.name);
